...
from PyQt6.QtWidgets import (
    ...
    QApplication, QSizePolicy, QComboBox, QStyleFactory
)
...
class CompareStringsWidget(QWidget):
    def __init__(self, missing_keys, original_strings, file2_path):
        ...
        # Settings (shared org/app name with MainWindow)
        self.settings = QSettings("Augustus", "TranslationComparator")
...
class MainWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # Dark mode toggle as *checkable* button
        self.dark_button = QPushButton("🌓")
        self.dark_button.setCheckable(True)
        self.dark_button.setFixedSize(32, 32)
        self.dark_button.setToolTip("Toggle forced dark mode on/off")
        bottom_bar.addWidget(self.dark_button)
        self.dark_button.toggled.connect(self.apply_dark_mode)

        # Style cycle button
        self.style_button = QPushButton("🎨")
        self.style_button.setFixedSize(32, 32)
        self.style_button.setToolTip("Cycle Qt styles")
        self.style_button.clicked.connect(self.cycle_style)
        bottom_bar.addWidget(self.style_button)
        ...
        # ---- Load stored theme state ----
        saved_style = self.settings.value("style", "Windows")
        self.apply_style(saved_style)

        dark_enabled = self.settings.value("darkmode_enabled", False, type=bool)
        self.dark_button.setChecked(dark_enabled)
    ...
    def apply_dark_mode(self, enabled: bool):
        ...
        if enabled:
            ...
            palette.setColor(QPalette.ColorRole.Highlight, QColor(85,140,220))
        ...
        self.settings.setValue("darkmode_enabled", enabled)
    ...
    def cycle_style(self):
        available = [str(s) for s in QStyleFactory.keys()]
        current = self.settings.value("style", "Windows")
        idx = (available.index(current) + 1) % len(available) if current in available else 0
        self.apply_style(available[idx])
        QMessageBox.information(self, "Style Changed", f"Switched to: {available[idx]}")
    ...
    def export_missing_keys(self):
        try:
            enc1 = self.file1_encoding_box.currentText()
            enc2 = self.file2_encoding_box.currentText()
            with open(self.file1_path, 'r', encoding=enc1) as f1:
                content1 = f1.read()
            with open(self.file2_path, 'r', encoding=enc2) as f2:
                content2 = f2.read()
            strings1 = extract_all_strings(content1)
            strings2 = extract_all_strings(content2)
            missing_keys = [k for k in strings1 if k not in strings2]
            if not missing_keys:
                ...
                return
            export_path, _ = QFileDialog.getSaveFileName(self, "Save As", "missing_keys.txt", "Text (*.txt)")
            if not export_path:
                return
            with open(export_path, 'w', encoding='utf-8') as out:
                for key in missing_keys:
                    value = strings1[key].replace('"', r'\"')
                    out.write(f'{{{key}, "{value}"}},\n')
            QMessageBox.information(self, "Exported", export_path)
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))

if __name__ == "__main__":
    # Load settings before app instance
    settings = QSettings("Augustus", "TranslationComparator")
    darkmode_enabled = settings.value("darkmode_enabled", False, type=bool)

    app = QApplication(sys.argv)

    # Ensure default style is Windows (not WindowsVista/Fusion)
    available = [str(s) for s in QStyleFactory.keys()]
    default_style = settings.value("style", "Windows")
    if default_style not in available:
        default_style = "Windows"
    app.setStyle(default_style)

    # Apply forced palette if darkmode is enabled
    if darkmode_enabled:
